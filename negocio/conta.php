<?php
class conta extends generico {
    public function obterTodos($data_inicio = '', $data_fim = '') {
        $sql_data = '';
        if (!empty($data_inicio)) {
            $data_inicio = funcoes::desformatarData($data_inicio);
            $sql_data .= " and data_pagamento >= '{$data_inicio}' ";
        }
        if (!empty($data_fim)) {
            $data_fim = funcoes::desformatarData($data_fim);
            $sql_data .= " and data_pagamento <= '{$data_fim}' ";
        }
        //var_dump($sql_data);
        //die();
        return $this->getByCondicao("WHERE true {$sql_data} ORDER BY data_pagamento DESC");
    }
    
    public function obterTodosByParametros1($pago, $inicio, $fim) {
        ...
        $sql_fim = '';
        if (!empty($pago)) {
            $sql_pago = " and pago = {$pago} ";
        }
        if (!empty($inicio)) {
            $sql_inicio = " and data_pagamento >= '{$inicio} 00:00:00' ";
        }
        if (!empty($fim)) {
            $sql_fim = " and data_pagamento <= '{$fim} 23:59:59' ";
        }
        return $this->getByCondicao("WHERE true {$sql_pago} {$sql_inicio} {$sql_fim} ORDER BY data_pagamento");
    }
